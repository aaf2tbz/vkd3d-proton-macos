#!/bin/bash
# release ship: assemble the fully-working implementation tarball.
# Usage: bash scripts/ship-runtime.sh
...
WS="$(cd "$(dirname "$0")/.." && pwd)"
DATE="$(date +%Y-%m-%d)"
STAGE="$(mktemp -d /tmp/vkd3d-release-ship.XXXXXX)"
