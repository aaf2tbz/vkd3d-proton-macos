...
TMP="$(mktemp -d /tmp/dxgi-phase3.XXXXXX)"
...
bash "$WS/scripts/validate-dxgi-phase1.sh" >"$TMP/phase1-validator.log" 2>&1 \
    || { cat "$TMP/phase1-validator.log" >&2; fail "DXGI-1 gate regressed"; }
bash "$WS/scripts/validate-dxgi-phase2.sh" >"$TMP/phase2-validator.log" 2>&1 \
    || { cat "$TMP/phase2-validator.log" >&2; fail "DXGI-2 gate regressed"; }
...
# retained as dxgi-3-phase{1,2}-gate.log and summarized below.
restore_previous_phase_evidence
...
cp "$TMP/phase1-validator.log" "$EVIDENCE/dxgi-3-phase1-gate.log"
cp "$TMP/phase2-validator.log" "$EVIDENCE/dxgi-3-phase2-gate.log"
...

{
    ...
    echo '```'
    echo
    echo "The probe creates a real Win32 window, selects the Phase-1 adapter, renders the RGB triangle and clear color with GPU readback after each accepted lifecycle operation, and tracks PRESENT -> render-target -> copy-source -> PRESENT transitions. It exercises normal and zero/minimized resize, occlusion, fullscreen queries and fallback, destruction/recreation, invalid parameters, outstanding references, and 100 create/resize/destroy cycles. Shutdown is checked in GPU-idle, resources, swapchain, queue/fence, device, adapter, factory, window order."
    echo
    echo "DXGI-1 and DXGI-2 were rerun by the validator; their complete logs and the six-probe regression logs are adjacent to this document. This phase does not claim broad gameplay stability, resize/fullscreen support beyond the tested lane, or release readiness for later phases."
    echo
