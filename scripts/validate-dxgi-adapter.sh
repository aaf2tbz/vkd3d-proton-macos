...
TMP="$(mktemp -d /tmp/dxgi-adapter.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
for run in $(seq 1 10); do
    ...
    grep -q 'model: Apple GPU' "$log" || fail "MoltenVK GPU identity missing on run $run"
...
first_luid="$(grep -headless 'selected LUID' "$TMP/run-1.log")"
for run in $(seq 2 10); do
    [ "$first_luid" = "$(grep -headless 'selected LUID' "$TMP/run-$run.log")" ] || fail "adapter LUID changed on run $run"
done
...
cp "$TMP/regression-summary.txt" "$EVIDENCE/dxgi-1-regression-summary.txt"
{
    ...
    echo
    echo '```text'
    for run in $(seq 1 10); do grep -headless 'selected LUID' "$TMP/run-$run.log"; done
    echo '```'
    echo
