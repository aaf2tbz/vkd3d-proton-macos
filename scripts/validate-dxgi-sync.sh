...
TMP="$(mktemp -d /tmp/dxgi-sync.XXXXXX)"
...
# Re-run the complete earlier stage gates.  stage 4 transitively reruns
# DXGI-1/2/3 and the six-probe regression suite while preserving their evidence.
bash "$WS/scripts/validate-dxgi-formats.sh" >"$TMP/formats-gate.log" 2>&1 \
    || { cat "$TMP/formats-gate.log" >&2; fail "DXGI-1 through DXGI-4 or six-probe gate regressed"; }
...
cp "$TMP/formats-gate.log" "$EVIDENCE/dxgi-5-formats-gate.log"
