...
TMP="$(mktemp -d /tmp/dxgi-formats.XXXXXX)"
...
[ -s "$DXVK_FORMATS_PATCH" ] || fail "DXGI-4 formats patch missing"
...
bash "$WS/scripts/validate-dxgi-lifecycle.sh" >"$TMP/lifecycle-validator.log" 2>&1 \
    || { cat "$TMP/lifecycle-validator.log" >&2; fail "DXGI-3 gate regressed"; }
restore_previous_stage_evidence
...
cp "$TMP/lifecycle-validator.log" "$EVIDENCE/dxgi-4-lifecycle-gate.log"
...

{
    ...
    echo "- DXGI-4 formats patch: $DXVK_FORMATS_PATCH"
    echo "- DXGI-4 formats patch SHA-256: $(shasum -a 256 "$DXVK_FORMATS_PATCH" | awk '{print $1}')"
    ...
    echo
    echo "The full two-run Wine/vkd3d/MoltenVK output is in dxgi-4-probe-run.txt and dxgi-4-probe-repeat.txt. DXGI-3 preservation output is in dxgi-4-lifecycle-gate.log. No broad gameplay-stability claim is made."
