...
**Date:** 2026-08-14
**Status:** ACTIVE — Phase 0 complete (evidence locked), M0 in progress
...
| **M7b** | Ray-query dispatch slice | 🔶 IN PROGRESS 2026-08-14 — AS descriptor support + TLAS build (MTL4 indirect instances) + SPIRV-Cross MSL fixes (AS texture-slot index, committed→candidate getters, no -1 type offset) landed and committed (MoltenVK ec208d7, SPIRV-Cross f741ee49). macOS 26 beta workarounds probe-verified: CPU-written heap data invisible to MTL4 AS builds (staging), heap AS un-traversable (standalone AS), heap scratch un-traversable (standalone scratch), classic encoder can't resolve AS from raw argument data (MTL4 argument-table dispatch). BLOCKER: SPIRV-Cross emits the argument buffer at [[buffer(0)]], which collides with the MTL4 argument table's own slot — the dispatch runs but returns no hits; next step is the argument-buffer binding index shift. Evidence: 2026-08-14-m7-rayquery-dispatch-slice.md |
