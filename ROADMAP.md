...
| **M7b** | Ray-query dispatch slice | ✅ GREEN 2026-08-14 — the FULL Vulkan inline ray query works end to end: discrete AS descriptor sets (AS as a direct [[buffer(N)]] resource), MTL4 argument-table dispatch (setResource/setAddress from the bind script), SPIRV-Cross AS→msl_buffer + candidate getters + no -1 type offset, standalone output buffers (heap-backed UAV writes are flaky on the beta). Probe: 2107/4096 hits, distances read back; follow-ups: distance ~2x discrepancy vs pure Metal (10.97 vs 5.43), heap-backed UAV staging for real DXR. Commits: MoltenVK cad86eb, SPIRV-Cross fc6cae47. Evidence: 2026-08-14-m7-rayquery-full-path.md |
