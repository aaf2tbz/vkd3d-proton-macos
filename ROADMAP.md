...
| **M7b** | Ray-query dispatch slice | 🔶 INTEGRATED 2026-08-14 — the full dispatch path is landed and the pattern is pure-Metal VERIFIED (30/256 hits, exact distances 5.000-5.431, 3/3+ runs): discrete AS descriptor sets (AS as a direct [[buffer(N)]] resource), MTL4 argument-table dispatch (setResource/setAddress from the bind script), SPIRV-Cross AS→msl_buffer + candidate getters + no -1 type offset. BLOCKER: the MVK exposes no standalone MTLStorageModeShared memory type — the type-1 (placement-heap) and type-0 (private) outputs cannot be written by MTL4 argument-table dispatches (probe honestly reads 0). Next: MVK output staging (standalone shared staging buffers) or a standalone-shared memory type; then distance verification + vkd3d DXR 1.1 (M8). Commits: MoltenVK cad86eb, SPIRV-Cross fc6cae47. Evidence: 2026-08-14-m7-rayquery-full-path.md |
